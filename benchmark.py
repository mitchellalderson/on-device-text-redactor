...
image = (
    ...
    .env({"LD_LIBRARY_PATH": "/usr/local/lib/python3.12/site-packages/nvidia/cudnn/lib:/usr/local/cuda/lib64:/usr/local/lib/python3.12/site-packages/nvidia/cublas/lib"})
    .apt_install("wget")
...
def benchmark(revisions: list[str], samples: int, seed: int, failures: int, output_file: str | None):
    import numpy as np
    ...
    def load_tokenizer(rev):
        from huggingface_hub import hf_hub_download
        tok_path = hf_hub_download(repo_id=HF_REPO, filename="tokenizer.json", revision=rev)
        config_path = hf_hub_download(repo_id=HF_REPO, filename="tokenizer_config.json", revision=rev)
    ...
    def run_inference(session, tok, input_text):
        ...
        for _ in range(MAX_NEW_TOKENS):
            ...
            outputs = session.run(None, {"input_ids": ids_arr})
    ...
    def run_bench(session, tok, data, label):
        ...
        for i, sample in enumerate(data):
            output, wall, ntok = run_inference(session, tok, sample["input"])
            metrics = {
            ...
            total_tokens += ntok
            if (i + 1) % 10 == 0:
                print(f"    {i + 1}/{len(data)} done ({(i + 1) / len(data) * 100:.0f}%)")
    ...
    onnx_a = hf_hub_download(repo_id=HF_REPO, filename="model_fp16.onnx", revision=rev_a)
    ...
    onnx_b = hf_hub_download(repo_id=HF_REPO, filename="model_fp16.onnx", revision=rev_b)
    ...
    for label, v1, v2 in [
        ("Exact Match", f"{results_a['exact_match_rate']*100:.1f}%", f"{results_b['exact_match_rate']*100:.1f}%"),
        ("Char F1", f"{results_a['avg_char_f1']:.3f}", f"{results_b['avg_char_f1']:.3f}"),
        ("REDACTED Recall", f"{results_a['avg_redacted_recall']:.3f}", f"{results_b['avg_redacted_recall']:.3f}"),
        ("REDACTED Precision", f"{results_a['avg_redacted_precision']:.3f}", f"{results_b['avg_redacted_precision']:.3f}"),
        ("Avg Tokens/sec", f"{results_a['avg_tps']:.1f}", f"{results_b['avg_tps']:.1f}"),
        ("Avg Time (s)", f"{results_a['avg_wall_time']:.2f}", f"{results_b['avg_wall_time']:.2f}"),
    ]:
        print(f"  {label:<20}{v1:>18}{v2:>18}")

    print("-" * w)
    print(f"  {'Total Time (s)':<20}{results_a['total_time']:>18.1f}{results_b['total_time']:>18.1f}")
    print("=" * w)
    ...
    if failures > 0:
        ...
        for res in [results_b, results_a]:
            ...
            for rank, r in enumerate(worst, 1):
                print(f"  #{rank}  F1: {r['char_f1']:.3f}  R/P: {r['redacted_recall']:.2f}/{r['redacted_precision']:.2f}")
    ...
    if output_file:
        with open(output_file, "w") as f:
            json.dump({"a": results_a, "b": results_b, "config": {
                "samples": samples, "seed": seed, "revisions": revisions,
            }}, f, indent=2)
